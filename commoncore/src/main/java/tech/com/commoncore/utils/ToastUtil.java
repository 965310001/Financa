...
public class ToastUtil {
    ...
    public static void show(CharSequence content) {
        if (content.toString().contains("NETWORK_ERROR")) {
            content = "网络不好，请稍后再试！";
        } else if (content.toString().contains("TIMEOUT_ERROR")) {
            content = "连接超时，请稍后再试！";
        } else if (content.toString().contains("PARSE_ERROR")) {
            content = "解析错误，请稍后再试！";
        } else if (content.toString().contains("SSL_ERROR")) {
            content = "证书出错，请稍后再试！";
        } else if (content.toString().contains("UNKNOWN")) {
            content = "未知错误，请稍后再试！";
        }
        show(content, sIsShowRunningForeground);
    }
    ...
    public static void show(CharSequence content, boolean isShowRunningForeground) {
        show(content, isShowRunningForeground, getBuilder().setGravity(Gravity.CENTER));
    }
